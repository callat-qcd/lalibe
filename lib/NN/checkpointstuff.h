...
class checkpoint{
 ...
 checkpoint(const std::string& filenamee, const int& stripesizee) : isopen(false), stripesize(stripesizee), mainpath(""){
    ...
    //setting parameters:
    template<class T>
        int set_parameter(const std::string& paramname, const T& val, HDF5Base::writemode & h5mode){
        if(!isopen) open(HDF5Base::ate);
    
        QDPIO::cout<< "writing parameter " << paramname << std::flush;
        file.write(mainpath+paramname,val, h5mode);
    ...
    template<class T>
        int set_parameter(const std::string& paramname, const multi1d<T>& arrayval, HDF5Base::writemode & h5mode){
        if(!isopen) open(HDF5Base::ate);

        QDPIO::cout<< "writing parameter " << paramname << std::flush;
        file.write(mainpath+paramname,arrayval, h5mode);
        QDPIO::cout << " done!" << std::endl;
        return EXIT_SUCCESS;
    }
    
    template<class T>
        int set_parameter(const std::string& paramname, const multi2d<T>& array2val, HDF5Base::writemode & h5mode){
        if(!isopen) open(HDF5Base::ate);

        QDPIO::cout<< "writing parameter " << paramname << std::flush;
        file.write(mainpath+paramname,array2val, h5mode);
        QDPIO::cout << " done!" << std::endl;
        return EXIT_SUCCESS;
    }
    
    template<class T>
        int set_parameter(const std::string& paramname, const PSeed<T>& seed, HDF5Base::writemode & h5mode){
        if(!isopen) open(HDF5Base::ate);
        multi1d<T> readseed(4);
        for(unsigned int i=0; i<4; i++) readseed[i]=seed.elem(i);
        file.write(mainpath+paramname,readseed, h5mode);
        return EXIT_SUCCESS;
    }
...
//setting
template<>
int checkpoint::set_parameter<Complex>(const std::string& paramname, const multi1d<Complex>& arrayval, HDF5Base::writemode & h5mode);
template<>
int checkpoint::set_parameter<int>(const std::string& paramname, const int& val, HDF5Base::writemode & h5mode);
template<>
int checkpoint::set_parameter<unsigned int>(const std::string& paramname, const unsigned int& val, HDF5Base::writemode & h5mode);
template<>
int checkpoint::set_parameter<LatticePropagatorD3>(const std::string& paramname, const LatticePropagatorD3& val, HDF5Base::writemode & h5mode);
template<>
int checkpoint::set_parameter<LatticePropagatorF3>(const std::string& paramname, const LatticePropagatorF3& val, HDF5Base::writemode & h5mode);
//getting
template<>
