// Arjun has literally copied this in to overload the momentum_list function.
...
namespace Chroma 
{
  ...
  // Param struct for LalibeSftMom
  LalibeSftMomParams_t::LalibeSftMomParams_t()
  {
  ...
  LalibeSftMom::LalibeSftMom(int mom2_max, bool avg_mom, int j_decay)
  {
  ...
  //To match header file, this is commented out. Only origin version allowed.
  /*LalibeSftMom::LalibeSftMom(const multi2d<int> & moms , int j_decay)
  {
    decay_dir = j_decay;
  ...
  //Arjun copied function above, but with origin offset.
  LalibeSftMom::LalibeSftMom(const multi2d<int> & moms , multi1d<int> origin_offset_, int j_decay)
  {
    decay_dir = j_decay;
		
    multi1d<int> orig = origin_offset_;
  ...
  LalibeSftMom::LalibeSftMom(int mom2_max, multi1d<int> origin_offset_, bool avg_mom,
                 int j_decay)
  {
    multi1d<int> mom_off;
  ...
  void
  LalibeSftMom::init(int mom2_max, multi1d<int> origin_off, multi1d<int> mom_off,
	       bool avg_mom, int j_decay)
  {
	...
	if (mom_num < 0) {
	  QDP_error_exit("LalibeSftMom: mom_num < 0. Shouldn't be here.\n") ;
	}
	...
	for (int mu=0; mu < mom_size.size(); ++mu) {
	  if (mom[mu] != mom_list[mom_num][mu]) {
	    ...
	    QDP_error_exit("LalibeSftMom: (mom != mom_list[mom_num])\n") ;
	  }
	}
  ...
  multi1d<int> 
  LalibeSftMom::canonicalOrder(const multi1d<int>& mom) const
  {
  ...
  int 
  LalibeSftMom::momToNum(const multi1d<int>& mom_in) const
  {
    multi1d<int> mom;
  ...
  multi2d<DComplex>
  LalibeSftMom::sft(const LatticeComplex& cf) const
  {
  ...
  multi2d<DComplex>
  LalibeSftMom::sft(const LatticeComplex& cf, int subset_color) const
  {
  ...
  multi2d<DComplex>
  LalibeSftMom::sft(const LatticeReal& cf) const
  {
    multi2d<DComplex> hsum(num_mom, sft_set.numSubsets()) ;
  ...
  multi2d<DComplex>
  LalibeSftMom::sft(const LatticeReal& cf, int subset_color) const
  {
    int length = sft_set.numSubsets();
    multi2d<DComplex> hsum(num_mom, length);
  ...
  multi2d<DComplex>
  LalibeSftMom::sft(const LatticeComplexD& cf) const
  {
    multi2d<DComplex> hsum(num_mom, sft_set.numSubsets()) ;
  ...
  multi2d<DComplex>
  LalibeSftMom::sft(const LatticeComplexD& cf, int subset_color) const
  {
    int length = sft_set.numSubsets();
    multi2d<DComplex> hsum(num_mom, length);
