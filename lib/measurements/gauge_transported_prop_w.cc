...
namespace Chroma
{
    namespace LalibeGaugeTransportedPropagatorEnv
    {
	   ...
	   for (int mu = 0; mu < Nd; mu++)
	   {
	       ...
	       LatticeFermion soln_ferm = zero;
	       PropToFerm(shifted_quark_propagator, soln_ferm, color_source, spin_source);
	     ...
	     TheNamedObjMap::Instance().getData<LatticePropagator>(params.named_obj.prop_id) = quark_soln;
