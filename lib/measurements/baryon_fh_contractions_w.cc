...
namespace Chroma
{
    namespace LalibeBaryonFHContractionsEnv
    {
        namespace
        {
            ...
            {
                return new InlineMeas(FHBaryonParams(xml_in, path));
            }

            bool registered = false;
        }
        const std::string name = "BARYON_FH_CONTRACTIONS";
        ...
        void read(XMLReader& xml, const std::string& path, FHBaryonParams::Param_t& par)
        {
            ...
            // Get list of currents and flavors
            read(paramtop, "currents", par.currents );
            read(paramtop, "flavors",  par.flavors  );
        }


        void write(XMLWriter& xml, const std::string& path, const FHBaryonParams::Param_t& par)
        {
            push(xml, path);
            ...
            write(xml, "currents", par.currents );
            write(xml, "flavors",  par.flavors  );

            pop(xml);
        }

        // NamedObject input
        void read(XMLReader& xml, const std::string& path, FHBaryonParams::NamedObject_t& input)
        {
            ...
            // Read FH props
            read(inputtop, "fh_quarks"   , input.fh_quarks);
        }

        void write(XMLWriter& xml, const std::string& path, const FHBaryonParams::NamedObject_t& input)
        {
            push(xml, path);
            ...
            // FH props
            write(xml, "fh_quarks"   , input.fh_quarks);
            pop(xml);
        }


        FHBaryonParams::FHBaryonParams()
        {
            frequency = 0;
        }

        FHBaryonParams::FHBaryonParams(XMLReader& xml_in, const std::string& path)
        {
            try
                {
                    ...
                    read(paramtop, "FHBaryonParams", param);
        ...
        void
        FHBaryonParams::writeXML(XMLWriter& xml_out, const std::string& path)
        {
            push(xml_out, path);

            write(xml_out, "FHBaryonParams", param);
        ...
        void  InlineMeas::operator()(unsigned long update_no, XMLWriter& xml_out)
        {
            ...
            if( params.param.currents.size() != params.named_obj.fh_quarks.size() || params.param.flavors.size() != params.named_obj.fh_quarks.size()){
                QDPIO::cout<<"you must provide an equal number of currents, flavors and fh_quarks in matching order"<<std::endl;
                QDPIO::cerr<<"you must provide an equal number of currents, flavors and fh_quarks in matching order"<<std::endl;
                QDP_abort(1);
            }
                    ...
                    try
                        {
                            ...
                            if (prop_record_xml.count("/Propagator") != 0)
                                ...
                                {
                                    QDPIO::cout<<"What type of weird propagator did you give me?"
                                               <<"  I can't find the right tag to get j_decay, source location, and whatnot...exiting..."<<std::endl;
                                }
                            // set info
                            if (!checkFlag) {
            ...
            for (auto aParticle : params.param.particle_list){
                ...
                // Loop over FH_props
                LatticePropagator FHprop;
                char FHflav;
                for (int iFH=0; iFH < params.param.currents.size(); iFH++){

                    QDPIO::cout<<params.named_obj.fh_quarks[iFH] <<" current: "<< params.param.currents[iFH]
                               <<" flavor: "<<params.param.flavors[iFH]<<std::endl;
                    FHprop = TheNamedObjMap::Instance().getData<LatticePropagator>(params.named_obj.fh_quarks[iFH]);
                    // Rotate to Dirac Basis?
                    if(params.param.rotate_to_Dirac == true)
                        rotate_to_Dirac_Basis(FHprop);
                    // This hack only supports flavor conserving currents for now
                    FHflav = tolower(params.param.flavors[iFH][0]);
                
                    LatticeComplex baryon = zero;
                    LatticeComplex FHtmp  = zero;
                    int FH_contract = 0;
                    if( std::get<0>(flavCode) == FHflav ){
                        QDPIO::cout<<"     swap first quark"<<std::endl;
                        FH_contract += 1;
                        do_contraction(FHprop,
                    ...
                    if( std::get<1>(flavCode) == FHflav ){
                        QDPIO::cout<<"     swap second quark"<<std::endl;
                        FH_contract += 1;
                        FHtmp = zero;
                    ...
                    if( std::get<2>(flavCode) == FHflav ){
                        QDPIO::cout<<"     swap third quark"<<std::endl;
                        FH_contract += 1;
                        FHtmp = zero;
                        do_contraction(prop_map[std::get<0>(flavCode)],
                                       prop_map[std::get<1>(flavCode)],
                    ...
                    // only write correlator if we did some contractions
                    if( FH_contract > 0){
                        write_correlator(params.param.output_full_correlator, params.param.is_antiperiodic,
                                         "fh_"+aParticle.first+"_"+params.param.currents[iFH]+"_"+params.param.flavors[iFH], aParticle.second,
#ifdef BUILD_HDF5
