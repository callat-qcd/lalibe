...
#ifndef __lalibe_stochastic_fh_prop_w_h__
#define __lalibe_stochastic_fh_prop_w_h__
...
{ 
  namespace LalibeStochasticFHPropagatorEnv
  {
    ...
    struct StochasticFHParams 
    {
      StochasticFHParams();
      StochasticFHParams(XMLReader& xml_in, const std::string& path);
      void writeXML(XMLWriter& xml_out, const std::string& path);

      unsigned long      frequency;

      struct StochasticFHProp_t{
	...
	multi1d<std::string>            noise_prop_id;
    ...
    public:
      ~InlineMeas() {}
      InlineMeas(const StochasticFHParams& p) : params(p) {}
