                ...
                for(int nu=mu+1; nu<Nd;++nu)
                    {
                        ...
                        ttmp = shift(u[mu],BACKWARD,mu);
                        t2   = shift(ttmp ,FORWARD,nu);
                        ...
                        t1   = shift(u[mu],BACKWARD,mu);
                        ttmp = shift(u[nu],BACKWARD,mu);
                        t2   = shift(ttmp ,BACKWARD,nu);
                        t3   = t2*t1;
                        t4   = adj(t3);
                        ttmp = shift(u[mu],BACKWARD,mu);
                        t1   = shift(ttmp ,BACKWARD,nu);
                        t2   = shift(u[nu],BACKWARD,nu);
                        t3   = t1*t2;
                        F[ctr] += t4*t3;
                        //  U_4 = u^dag(x-nu,nu)*u(x-nu,mu)*u(x-nu+mu,nu)*u^dag(x,mu)
                        t1   = shift(u[mu],BACKWARD,nu);
                        ttmp = shift(u[nu],BACKWARD,nu);
                        t2   = shift(ttmp ,FORWARD,mu);
                        t3   = t1*t2;
                        t4   = adj(u[mu]);
                        t1   = t3*t4;
                        t2   = shift(u[nu],BACKWARD,nu);
