...
def fh_flavor_terms(isospin, isospin_z, strangeness, fh_flavor):
    flavors, coeffs = flavor_terms(isospin, isospin_z, strangeness)
    ...
    for term in flavors:
        ...
        for i,f in enumerate(term):
            if f==fh_flavor:
                ind.append(i)
        fh_indices.append(ind)
    
    fh_flavors = []
    for i in range(len(fh_indices[0])):
        flavors_copy = list(flavors)
        n = 0
        for terms in flavors_copy:
            for j,f in enumerate(terms):
                if f==fh_flavor:
                    if n==i:
                        terms[j] = 3
                    n += 1
        fh_flavors.append(flavors_copy)

    return fh_flavors, coeffs
...
'''
def baryon_correlator(isospin, isospin_z, strangeness, spin, spin_z, parity):
    ...
    def correlator(q1, q2, q3):
        return two_point_correlator(q1, q2, q3, flavors, flavor_coeffs, spins, spin_coeffs)

    return correlator

def fh_correlator(isospin, isospin_z, strangeness, spin, spin_z, parity, fh_flavor):

    flavor_set, flavor_coeffs = fh_flavor_terms(isospin, isospin_z, strangeness, fh_flavor)
    spins, spin_coeffs = spin_terms(isospin, strangeness, spin, spin_z, parity)

    def correlator(q1, q2, q_fh):
        out = np.zeros(q1.shape[:4], dtype=np.complex128)

        for flavors in flavor_set:
            out += two_point_correlator(q1, q2, q_fh, flavors, flavor_coeffs, spins, spin_coeffs)
...
def two_point_correlator(q1, q2, q3, flavors, flavor_coeffs, spins, spin_coeffs):
...
def flavor_vector(isospin_z, strangeness):
    ...
    flavors[:up] = 0
    if strangeness != 0: flavors[strangeness:] = 2
